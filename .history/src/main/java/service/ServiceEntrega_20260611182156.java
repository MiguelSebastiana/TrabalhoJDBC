...
public class ServiceEntrega {
    ...
    public Entrega serviceGerarEntrega(int pedidoId, int motoristaId, Date dataEntrega) throws SQLException{
        ...
        Entrega entrega = new Entrega(pedidoId, motoristaId, Date.valueOf(LocalDate.now()), dataEntrega, StatusEntrega.EM_ROTA);
    ...
    public void serviceAtualizarStatus(int entregaId, StatusEntrega novoStatus) throws SQLException, IllegalAccessException {
        ...
        if(novoStatus == StatusEntrega.ENTREGUE){
            pedidoDAO.atualizarStatusPedido(entrega.getPedido_id(), StatusPedido.ENTREGUE);
        }
    }
